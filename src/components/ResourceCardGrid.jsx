...
const fetchEvents = async () => {
  return [
    ...
    },
    {
      ...
      expected: 1200,
      paragraph: "Lorem ipsum dolor sit amet consectetur.",
      status: ["Sold Out", "Available", "Available"],
...
export default function ResourceCardGrid() {
  ...
  useEffect(() => {
    fetchEvents().then((res) => {
      setData(res);
      setOriginalData(res); // ✅ save original
    });
  }, []);
  ...
  return (
    <section className="ResourceCardGrid relative bg-[linear-gradient(180deg,rgba(255,255,255,0.15)_0%,rgba(113,197,232,0.15)_60.5%)]">
      <div className="container">
        ...
        {/* 🔍 FILTERS */}
        <div className="flex flex-wrap gap-[20px] mb-[30px] pt-[10px]">
          <input
            type="text"
            placeholder="Search"
            onChange={(e) => setSearch(e.target.value)}
            className="border px-[20px] py-[10px] rounded-full w-[300px]"
          />
                ...
                <div className="content-wrap px-[30px] py-[20px]">
                  ...
                  {item.status.map((stat, i) => (
                    <div key={i} className="flex justify-between mt-[10px]">
                      ...
                      <span
                        onClick={() => handleClick(cardIndex, i)}
